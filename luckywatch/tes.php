...
$xevil_api_key = getenv('XEVIL');
if (!$xevil_api_key) {
    $xevil_api_key = readline('Xevil API Key: ');
}
if (!$xevil_api_key) {
    exit;
}
...
function print_divider() {
    echo str_repeat("━", DIVIDER_WIDTH) . PHP_EOL; }
...
function watch_videos(array $base_headers, $apiKey, $userAgent) {
...
function main() {
    global $cookie, $userAgent, $xevil_api_key;
    ...
    while (true) {
        ...
        switch ($choice) {
            case '1':
                watch_videos($headers, $xevil_api_key, $userAgent);
                break;
            case '2':
                continue 2;
            case '3':
